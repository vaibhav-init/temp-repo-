...
class GlobalConfig:
  ...
  weathers = {}  # Dummy for training-only mode
  ...
  def __init__(self):
    ...
    # 0 if you don't want to concatenate, 1 to concatenate front and back
    # 2 to concatenate left and right sides along with front and back
    self.radar_cat = 1
    ...
    self.backbone = 'transFuser_cr'  # Vision backbone architecture used
    ...
    self.radar_architecture = 'regnety_032'  # LiDAR architecture used in the backbone
    ...
    # CARLA 0.9.16 CityObjectLabel -> model semantic class mapping
    # Model classes: 0=unlabeled, 1=vehicle, 2=road, 3=traffic_light,
    #                4=pedestrian, 5=road_line, 6=sidewalk
    self.converter = [
        0,  # 0  NONE        -> unlabeled
        2,  # 1  Roads       -> road
        6,  # 2  Sidewalks   -> sidewalk
        0,  # 3  Buildings   -> unlabeled
        0,  # 4  Walls       -> unlabeled
        0,  # 5  Fences      -> unlabeled
        0,  # 6  Poles       -> unlabeled
        3,  # 7  TrafficLight -> traffic_light
        0,  # 8  TrafficSigns -> unlabeled
        0,  # 9  Vegetation  -> unlabeled
        0,  # 10 Terrain     -> unlabeled
        0,  # 11 Sky         -> unlabeled
        4,  # 12 Pedestrians -> pedestrian
        4,  # 13 Rider       -> pedestrian
        1,  # 14 Car         -> vehicle
        1,  # 15 Truck       -> vehicle
        1,  # 16 Bus         -> vehicle
        1,  # 17 Train       -> vehicle
        1,  # 18 Motorcycle  -> vehicle
        1,  # 19 Bicycle     -> vehicle
        0,  # 20 Static      -> unlabeled
        0,  # 21 Dynamic     -> unlabeled
        0,  # 22 Other       -> unlabeled
        0,  # 23 Water       -> unlabeled
        5,  # 24 RoadLines   -> road_line
        0,  # 25 Ground      -> unlabeled
        0,  # 26 Bridge      -> unlabeled
        0,  # 27 RailTrack   -> unlabeled
        0,  # 28 GuardRail   -> unlabeled
    ]
    # Pad to 256 so any raw tag value can be used as an index
    self.converter.extend([0] * (256 - len(self.converter)))

    # BEV converter: maps the BEV semantic camera output (already remapped
    # by the data collector to C-Shenron BEV classes 0-10) back to itself.
    # This is an identity mapping because the data collector handles the
    # CARLA 0.9.16 tag -> BEV class conversion at collection time.
    self.bev_converter = [
        0,  # unlabeled
        1,  # road
        2,  # sidewalk
        3,  # lane_markers
        4,  # lane_markers broken
        5,  # stop_signs
    ...
    # Pad to 256 so any raw value can be used as an index
    self.bev_converter.extend([0] * (256 - len(self.bev_converter)))
    ...
    # Use the length of the weights array rather than the padded converter length
    self.num_bev_semantic_classes = len(self.bev_semantic_weights)
    ...
    # configs to toggle sensor use for model training

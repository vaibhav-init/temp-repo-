...
def main():
  ...
  parser.add_argument('--radar_channels', 
                      type=int, 
                      default=config.radar_channels,
                      help='How many channels in the radar array, channels here refer to radar views')
  parser.add_argument('--radar_cat', 
                      type=int, 
                      default=config.radar_cat, 
                      help="to use the radar concatenation technique")
  ...
  if args.radar_channels > 1:
    config.radar_channels = args.radar_channels
  
  # if args.radar_cat:
  #   config.radar_cat = 1
    # config.radar_cat = True
  config.radar_cat = args.radar_cat
  
  print("config.radar_cat == ", config.radar_cat)
  ...
  if bool(args.use_disk_cache):
    ...
    tmp_folder = tmp_folder + '/dataset_cache' + args.id
  ...
  def load_data_compute_loss(self, data, validation=False):
    ...
    if self.config.use_plant:
      ...
      pred_checkpoint, pred_future_bounding_box, _ = self.model(bounding_boxes=bounding_box_label,
                                    ...
                                    junction=junction,
                                    velocity=ego_vel)
    elif self.args.backbone in ('transFuser', 'aim', 'bev_encoder', 'transFuser_cr'):
      ...
      if self.config.use_radar:
        radar = data['radar'].to(self.device, dtype=torch.float32)
      else:
        radar = None

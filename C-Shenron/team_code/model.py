...
from transfuser_cr import TransfuserBackboneCR, TransformerDecoderLayerWithAttentionCR, TransformerDecoderWithAttentionCR
...
class LidarCenterNet(nn.Module):
  ...
  def __init__(self, config):
    ...
    elif self.config.backbone == 'transFuser_cr':
      self.backbone = TransfuserBackboneCR(config)
    else:
    ...
    if self.config.use_controller_input_prediction or self.config.use_wp_gru:
      if self.config.transformer_decoder_join:
        ...
        if self.config.tp_attention:
          ...
          decoder_layer = TransformerDecoderLayerWithAttentionCR(self.config.gru_input_size,
                                                               self.config.num_decoder_heads,
                                                               activation=nn.GELU())
          self.join = TransformerDecoderWithAttentionCR(decoder_layer,
  ...
  def forward(self, rgb, lidar_bev, radar, target_point, ego_vel, command):
    bs = rgb.shape[0]
    ...
    if self.config.backbone == 'transFuser':
      ...
      bev_feature_grid, fused_features, image_feature_grid = self.backbone(rgb, lidar_bev, radar)
    else:
      raise ValueError('The chosen vision backbone does not exist. '
                       'The options are: transFuser, aim, bev_encoder')

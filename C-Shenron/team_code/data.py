...
# PATCHED: Use relative path from team_code directory
sys.path.append(os.path.join(os.path.dirname(os.path.abspath(__file__)), 'sim_radar_utils'))
...
from convert2D_img import convert2D_img_func
...
class CARLA_Data(Dataset):  # pylint: disable=locally-disabled, invalid-name
        ...
        for seq in range(config.skip_first, num_seq - self.config.pred_len - self.config.seq_len):
          ...
          for idx in range(self.config.seq_len):
            if not self.config.use_plant:
              ...
              # radar.append(route_dir + '/radar_data_simulated_with_cos' + (f'/{(seq + idx):04}.npy'))
              # radar_back.append(route_dir + '/radar_data_simulated_invert_with_cos' + (f'/{(seq + idx):04}.npy'))
              ...
              # for radar data with 16 channels
              # radar.append(route_dir + '/radar_data_front_16_again' + (f'/{(seq + idx):04}.npy'))
              # radar_back.append(route_dir + '/radar_data_rear_16' + (f'/{(seq + idx):04}.npy'))
  ...
  def __getitem__(self, index):
    ...
    for i in range(self.config.seq_len):
      ...
      if not self.data_cache is None and cache_key in self.data_cache:
        ...
        if not self.config.use_plant:
          ...
          # radars_i = cv2.imdecode(radars_i, cv2.IMREAD_UNCHANGED)
      ...
      # Load data from the disc
        ...
        if not self.config.use_plant:
          ...
          if self.config.radar_channels == 2:
            ...
            # np.save(f'/radar-imaging-dataset/carla_garage_videos/check_train_data/{index}_radar.npy', radars_i)
          elif self.config.radar_cat == 2:
            # left right ko side by side concatenate karke (after proper rotation) and 
            # then usko front back ke concat ke saath directly add up karde toh it should work
            radar_front = convert2D_img_func(np.load(str(radars[i], encoding='utf-8')))
            radar_back = convert2D_img_func(np.load(str(radars_back[i], encoding='utf-8')))
            # radar_back = np.rot90(np.rot90(radar_back))
            # radar_cat_fb = np.concatenate((radar_front, radar_back), axis=0) 
            
            # # similarly for left and right side
            ...
            # radar_cat_lr = np.concatenate((radar_left, radar_right), axis=1)
            
            # # now first crop both the radar_cat_fb and radar_cat_lr and then add them up simply
            # radar_fb_crop = crop256X256(radar_cat_fb)
            # radar_lr_crop = crop256X256(radar_cat_lr)
            # radar_cat = radar_fb_crop + radar_lr_crop
    ...
    if not self.config.use_plant:
      ...
      processed_radar = loaded_radars[self.config.seq_len - 1]
      # pdb.set_trace()
      if self.config.radar_channels > 1:
        data['radar'] = processed_radar
      else:
        processed_radar = np.expand_dims(processed_radar, axis=2)  
        data['radar'] = np.transpose(processed_radar, (2, 0, 1))
      # np.save(f'./data_tmp/{i}.npy', data['radar'])

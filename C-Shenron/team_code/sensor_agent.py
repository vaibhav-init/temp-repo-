...
import gzip
import sys
sys.path.append('radar-imaging-dataset/carla-radarimaging/team_code/e2e_agent_sem_lidar2shenron_package/')
...
class SensorAgent(autonomous_agent.AutonomousAgent):
  ...
  def setup(self, path_to_conf_file, route_index=None):
    ...
    self.config.radar_channel = int(os.environ.get('RADAR_CHANNEL', 1))
    # pdb.set_trace()
    self.config.radar_cat = int(os.environ.get('RADAR_CAT', 0))
    # for concatenating all the four views
    self.config.radar_fblr = int(os.environ.get('RADAR_FBLR', 0))
    ...
    self.eval_dategen = int(os.environ.get('EVAL_DATAGEN', 0)) == 1
    
    if self.save_path is not None and self.eval_dategen:
      (self.save_path / 'lidar_eval').mkdir(exist_ok=True)
      (self.save_path / 'semantic_lidar_eval').mkdir(exist_ok=True)
      (self.save_path / 'rgb_eval').mkdir(exist_ok=True)
      (self.save_path / 'radar_img_eval').mkdir(exist_ok=True)
  ...
  def tick(self, input_data):
    ...
    for camera_pos in ['front']:
      ...
      if int(os.environ.get('BLACKOUT_IMAGE', 0)):
        camera = np.zeros((256, 1024, 3))
        # camera = np.random.rand(256, 1024, 3)
      # pdb.set_trace()
    ...
    if self.config.backbone not in ('aim'):
      ...
      result['semantic_lidar'] = t_u.semantic_lidar_to_ego_coordinate(self.config, input_data['semantic_lidar'])
  ...
  def save_data(self, rgb_front, lidar_eval, semantic_lidar_eval, radar_img):
    frame = self.step //self.config.data_save_freq
    
    '''
    saving data only for
    1. rgb_front -.jpg
    2. lidar - .laz
    3. semantic_lidar - .laz
    4. radar range-angle image - .npy
    '''
    
    cv2.imwrite(str(self.save_path / 'rgb_eval' / (f'{frame:04}.jpg')), rgb_front)
    
    # Specialized LiDAR compression format
    header = laspy.LasHeader(point_format=self.config.point_format)
    header.offsets = np.min(lidar_eval, axis=0)
    header.scales = np.array([self.config.point_precision, self.config.point_precision, self.config.point_precision])

    with laspy.open(self.save_path / 'lidar_eval' / (f'{frame:04}.laz'), mode='w', header=header) as writer:
      point_record = laspy.ScaleAwarePointRecord.zeros(lidar_eval.shape[0], header=header)
      point_record.x = lidar_eval[:, 0]
      point_record.y = lidar_eval[:, 1]
      point_record.z = lidar_eval[:, 2]
    ...
    header.offsets = np.min(semantic_lidar_eval[:, :3], axis=0)
    header.scales = np.array([self.config.point_precision, self.config.point_precision, self.config.point_precision])

    with laspy.open(self.save_path / 'semantic_lidar_eval' / (f'{frame:04}.laz'), mode='w', header=header) as writer:
      point_record = laspy.ScaleAwarePointRecord.zeros(semantic_lidar_eval.shape[0], header=header)
      point_record.x = semantic_lidar_eval[:, 0]
      point_record.y = semantic_lidar_eval[:, 1]
      point_record.z = semantic_lidar_eval[:, 2]
      point_record.cosine = semantic_lidar_eval[:, 3]
      point_record.index = semantic_lidar_eval[:, 4]
      point_record.sem_tag = semantic_lidar_eval[:, 5]
      # pdb.set_trace()
      writer.write_points(point_record)
    
    # save the radar_image here
    np.save(f'{self.save_path}/radar_img_eval/{frame:04}.npy', radar_img)
    return 0
  ...
  def run_step(self, input_data, timestamp, sensors=None):  # pylint: disable=locally-disabled, unused-argument
    ...
    if not self.initialized:
      ...
      if self.config.backbone not in ('aim'):
        ...
        self.semantic_lidar_last = deepcopy(tick_data['semantic_lidar'])
    ...
    if self.config.backbone not in ('aim'):
      ...
      semantic_lidar_last = self.align_semantic_lidar(self.semantic_lidar_last, ego_x_last, ego_y_last, ego_theta_last, ego_x, ego_y, ego_theta)
    ...
    if self.config.backbone not in ('aim'):
      ...
      # for semantic lidar concatenation
      semantic_lidar_current = deepcopy(tick_data['semantic_lidar'])
      semantic_lidar_full = np.concatenate((semantic_lidar_current, semantic_lidar_last), axis=0)
     
      self.semantic_lidar_buffer.append(semantic_lidar_full)
    ...
    if self.config.backbone in ('aim'):  # Image only method
      ...
      radar_list = []
      # pdb.set_trace()
      # prepare LiDAR input and RaDAR input
      for i in lidar_indices:
        ...
        if self.config.radar_cat == 2:
          ...
          # have all the four views here
          # back view
          radar_np_back = convert_sem_lidar_2D_img_func(raw_radar, 180)
          # radar_np_back = np.rot90(np.rot90(radar_np_back))
          # radar_cat_fb = np.concatenate((radar_np, radar_np_back), axis=0)
          # left view
          ...
          # radar_cat_lr = np.concatenate((radar_np_left, radar_np_right), axis=1)
          
          # # cropping both of them so that they can be added up
          # radar_fb_crop = crop256X256(radar_cat_fb)
          # radar_lr_crop = crop256X256(radar_cat_lr)
          # radar_np = radar_fb_crop + radar_lr_crop
          ...
          # np.save(f"{self.save_path}/radar_img_front_masked/{frame:04}.npy", radar_np)
          # np.save(f"{self.save_path}/radar_img_back_masked/{frame:04}.npy", radar_np_back)
          # np.save(f"{self.save_path}/radar_img_left_masked/{frame:04}.npy", radar_np_left)
          # np.save(f"{self.save_path}/radar_img_right_masked/{frame:04}.npy", radar_np_right)
        ...
        # to check if the log value is set in the environment or not
        ...
        if not (radar_channel > 1):
          radar_np_exp = np.expand_dims(radar_np, axis=2)
          radar_np_exp = np.transpose(radar_np_exp, (2, 0, 1))
        else:
          radar_np_exp = radar_np
        radar = torch.from_numpy(radar_np_exp)
        radar = radar.to(self.device, dtype=torch.float32)
        radar = radar.unsqueeze(0)
        radar_list.append(radar)
        radar_tensor = torch.cat(radar_list, dim=1)
        
        # if int(os,environment.get('BLACKOUT_IMAGE', 0)):
        
        # torch.cat???
        rgb_front = input_data['rgb_front'][1][:, :, :3]
        if self.eval_dategen:
          self.save_data(rgb_front, self.lidar_buffer[i], self.semantic_lidar_buffer[i], radar_np)
    ...
    for i in range(self.model_count):
      if self.config.backbone in ('transFuser', 'aim', 'bev_encoder', 'transFuser_cr'):
